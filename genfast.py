...
# 見た目の世界観を統一（シンセウェーブ／レトロ・アーケード）。画像不要・canvas 描画で実現。
VISUAL_STYLE = (
    "見た目は『シンセウェーブの夜のゲーセン』で統一する。画像ファイルは使わず canvas 描画のみ:\n"
    "・配色: 主役=マゼンタ〜ホットピンク(#ff3df0系)、ライン=電光シアン(#23e5ff)、"
    "空=ディープパープル→ピンクの縦グラデ、闇=紫紺の黒(#140d17)。"
    "骨格は『マゼンタ×シアン』の補色対比。高彩度・高コントラスト。中間色で濁さない。\n"
    "・構図: 中央奥に横ストライプの沈む夕日、地平線へ収束するパースのグリッド床。中央は抜けて被写体/UIを置く。\n"
    "・光と質感: ネオンの発光は ctx.shadowBlur+shadowColor のブルームで表現。"
    "画面端を落とすビネット、うっすらスキャンライン(レトロCRT感)。\n"
    "・ムード: ノスタルジック／高揚／夜／祝祭的。子どもの頃の薄暗いゲーセンのワクワクを現代ネオンで。\n"
...
DESIGN_SYS = (
    ...
    "面白さは数値調整で決まる。抽象的な気分でなく、builder がそのまま実装できる具体値まで決める。\n"
    "出力フォーマット（Markdown・各 1〜2 行）:\n"
    "## title  … 短い英語のゲーム名\n"
    "## core   … 何をするゲームか・面白さの核（どのイディオムで、何が溜まり何が弾けるか）\n"
    "## tension … 緊張がどう蓄積し、どの瞬間に解放されるか\n"
    "## params … 具体値: 初速/加速率/出現間隔(秒)/スコア配点/失敗条件/制限時間 を数値で\n"
    "## rules  … 勝敗/スコア/終了条件・時間とともにどう難化するか\n"
    "## controls … 矢印/Z/C にそれぞれ何を割り当てるか（3 つとも使う）\n"
    "前置き・説明・コードは書かない。"
)
